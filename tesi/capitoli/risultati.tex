...
qualità media più alta e rappresenta quindi il miglior compromesso per un impiego
...
contemporaneamente l'energia minima (24,6~J) e la qualità media massima (0,71), e
\emph{domina} quindi tutte le altre configurazioni, risultando l'unico punto sulla
frontiera di Pareto; ogni altra configurazione consuma di più a fronte di una
qualità media pari o inferiore. Le configurazioni Llama-3.2-3B, in particolare,
...
bit ottiene il punteggio massimo perché è simultaneamente la migliore su tutte e
tre le metriche --- energia più bassa, latenza più bassa e qualità media più alta
---, mentre le configurazioni Llama-3.2-3B, penalizzate da consumo ed energia
...
Durante la Campagna A la temperatura del SoC è stata monitorata dopo ogni
inferenza. Grazie alla ventola di raffreddamento attiva e alle pause di
raffreddamento introdotte tra le configurazioni, la temperatura si è mantenuta al
di sotto della soglia di \textit{throttling} ($\sim$80--85~\textcelsius), con un
massimo di \textbf{76,8}~\textcelsius\ e \textbf{nessun} evento di throttling
rilevato (Figura~\ref{fig:res-termico}). Le misure di latenza ed energia non sono
quindi state falsate da riduzioni di frequenza indotte dal calore. Va tuttavia
osservato che sessioni di misura molto prolungate, sommando il riscaldamento del
SoC alla sollecitazione continua in lettura della scheda microSD, possono
compromettere l'integrità dei file dei modelli memorizzati; per questo motivo la
procedura adotta la verifica di integrità all'avvio e le pause di raffreddamento
