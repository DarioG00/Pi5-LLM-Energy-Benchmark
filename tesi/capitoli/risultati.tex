...
risposte): latenza ed energia sono due \emph{costi}, entrambi da minimizzare. La
Figura~\ref{fig:res-pareto-thread} riporta la frontiera di Pareto nel piano
\textbf{latenza--energia} considerando tutte le configurazioni di modello e thread (le
migliori sono in basso a sinistra). La frontiera è occupata \emph{esclusivamente} da
configurazioni \textbf{qwen2.5-1.5B}: qwen Q8 a 1 thread (energia minima, 23,8~J), qwen
Q4 a 2 thread e qwen Q4 a 4 thread (latenza minima, 5,9~s); tutte le altre
configurazioni --- in particolare i modelli più grandi, a qualunque numero di thread ---
risultano \emph{dominate}. Restringendo l'analisi al singolo modello si ritrova quanto
osservato sulla latenza: le configurazioni a \textbf{4 thread} sono quasi sempre
dominate da quelle a 2 thread (consumano di più senza ridurre la latenza), e la scelta
razionale del parallelismo resta tra \textbf{1 thread} (energia minima, ma latenza
elevata) e \textbf{2 thread} (latenza minima, con un modesto sovrapprezzo energetico),
con i \textbf{2 thread} come miglior compromesso complessivo.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{imm/pareto_lat_energia.png}
    \caption{Frontiera di Pareto nel piano latenza--energia (Campagna B): tutte le
    configurazioni di modello e thread; la linea rossa collega le configurazioni non
    dominate. La frontiera è occupata dalle sole varianti qwen2.5-1.5B.}
