...
l'impatto energetico dei \textit{Large Language Model (LLM)}.

La tesi si inserisce nell'ambito della ricerca dell'IA sostenibile,
...
\begin{itemize}
    \item \textit{CommonsenseQA}~\cite{commonsenseqa}: benchmark finalizzato alla valutazione delle
    capacità di ragionamento basate sul senso comune attraverso quesiti a
    scelta multipla;

    \item \textit{BIG-Bench Hard}~\cite{bbh}: insieme di task complessi derivati dal
    benchmark BIG-Bench, utilizzato per misurare capacità avanzate di
    ragionamento e generalizzazione;

    \item \textit{TruthfulQA}~\cite{truthfulqa}: benchmark progettato per valutare la
    ...
    \item \textit{GSM8K}~\cite{gsm8k}: benchmark composto da problemi matematici elementari
    che richiedono ragionamento aritmetico articolato in più passaggi;

    \item \textit{HumanEval}~\cite{humaneval}: benchmark per la valutazione della generazione
