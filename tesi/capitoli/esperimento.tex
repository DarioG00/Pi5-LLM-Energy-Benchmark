...
Otii Ace Pro fino alla generazione dei grafici comparativi.
...
L'Otii Ace Pro campiona nel tempo la potenza assorbita sul canale di potenza
...
\subsection{Configurazione dello strumento Otii Ace Pro}

All'avvio dell'esperimento, lo script si connette a \textit{Otii Server} tramite
la \textit{Otii TCP Server API} e configura lo strumento Otii Ace Pro. La tensione di
alimentazione viene impostata a 5,1~V: tale valore, in linea con l'alimentatore ufficiale del Raspberry Pi 5, compensa la caduta di tensione lungo il cavo e l'adattatore USB-C, portando ai capi della scheda i 5~V richiesti ed evitando condizioni di \textit{undervoltage}; rientra comunque nella tolleranza di alimentazione del dispositivo (5~V $\pm$ 5\%). Il
...
computazionale intenso e compatibile con la corrente massima erogabile
dall'Otii Ace Pro, pari a 5~A; non è quindi necessaria alcuna alimentazione
esterna aggiuntiva.
...
misurata dalla registrazione dell'Otii Ace Pro, secondo
...
\begin{itemize}
    \item modulo di controllo dell'\textbf{Otii Ace Pro}: gestisce la connessione
...
configurazione dell'Otii Ace Pro, elenco dei modelli, numero di thread, numero di
...
alla potenza media assorbita, alla velocità di elaborazione (prompt processing e
generation, in token/s) e alla qualità delle risposte, per ciascun modello e
