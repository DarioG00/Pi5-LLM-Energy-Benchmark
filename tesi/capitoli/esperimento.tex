...
la forma:
\begin{center}
\texttt{llama-cli -m <modello>.gguf -t N -c 2048 -n 128}
\end{center}
...
\begin{itemize}
    ...
    \item \texttt{-c 2048}: definisce la dimensione della finestra di contesto, ossia
    il numero massimo di token che il modello può mantenere in memoria considerando
    sia il prompt sia la risposta generata. Il valore 2048 è ampiamente sufficiente a
    contenere ogni singolo prompt dei benchmark adottati e la relativa risposta,
    mantenendo al contempo un'occupazione di memoria trascurabile per un dispositivo
    con 8~GB di RAM come il Raspberry Pi 5;
...
Ogni prompt viene inoltre caricato tramite il comando \texttt{/read} di
\texttt{llama.cpp}, a partire da un file temporaneo scritto sul Raspberry Pi: in
questo modo se ne preservano gli a-capo. Ciò è essenziale per i task di
generazione di codice, in cui l'indentazione e la struttura su più righe del
prompt devono essere mantenute affinché il modello produca codice valido.
